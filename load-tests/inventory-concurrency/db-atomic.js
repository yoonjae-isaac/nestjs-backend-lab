...
export default function () {
  const response = http.post(
    `${baseUrl}/labs/inventory-concurrency/db-atomic/orders`,
...
export function teardown() {
  ...
  console.log(`DB_ATOMIC final state: ${state.body}`);
}

export function handleSummary(summary) {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  return {
    [`results/inventory-concurrency/db-atomic/${timestamp}.json`]: JSON.stringify(summary, null, 2),
    stdout: '\nDB_ATOMIC result written to results/inventory-concurrency/db-atomic\n',
  };
}

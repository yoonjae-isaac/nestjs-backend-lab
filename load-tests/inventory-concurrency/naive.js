...
const artificialDelayMs = Number.parseInt(__ENV.NAIVE_DELAY_MS ?? '10', 10);
...
export default function () {
  const response = http.post(
    `${baseUrl}/labs/inventory-concurrency/naive/orders?delayMs=${artificialDelayMs}`,
...
export function teardown() {
  ...
  console.log(`NAIVE final state: ${state.body}`);
}

export function handleSummary(summary) {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  return {
    [`results/inventory-concurrency/naive/${timestamp}.json`]: JSON.stringify(summary, null, 2),
    stdout: '\nNAIVE result written to results/inventory-concurrency/naive\n',
  };
}
